    ...
    <Row key={listUrl} align="center" padding="16px" id={listUrlRowHTMLId(listUrl)}>
      ...
      {isSelected ? (
        <ButtonPrimary
          ...
          style={{ width: '7rem', minWidth: '5rem', padding: '8px 32px', borderRadius: '12px', fontSize: '16px' }}
        >
          Selected
        </ButtonPrimary>
      ) : (
    ...
    <Column style={{ width: '100%', flex: '1 1' }}>
      <PaddedColumn>
        <RowBetween>
          ...
          <Text fontWeight={700} fontSize={32} style={{color:"#121127"}}>
            Manage Lists
          </Text>
      ...
      <PaddedColumn gap="14px">
        <Text fontWeight={700} fontSize={20}  style={{color:"#121127"}}>
          Add a list{' '}
          <QuestionHelper text={`Token lists are an open specification for lists of ${process.env.REACT_APP_TOKEN_PROTOCOL_NAME} tokens. You can use any token list by entering its URL below. Beware that third party token lists can contain fake or malicious ${process.env.REACT_APP_TOKEN_PROTOCOL_NAME} tokens.`} />
        </Text>
        <Row>
          <SearchInput
            type="text"
          ...
          />
          <AddListButton onClick={handleAddList} disabled={!validUrl} style={{color:"#000"}}>
            Add
          </AddListButton>
        </Row>
        {addError ? (
