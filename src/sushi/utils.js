...
export const getBusdStakingContract = (sushi, symbol) => {
  const stakingContract = symbol ? symbol.toLowerCase() + "Staking" : ""
  return sushi && sushi.contracts && sushi.contracts[stakingContract]
}

export const getAirdropContract = (sushi, symbol) => {
  const airdropContract = symbol ? symbol.toLowerCase() + "Airdrop" : ""
  return sushi && sushi.contracts && sushi.contracts[airdropContract]
}
...
export const getEarned = async (masterChefContract, pid, account) => {
  return masterChefContract.methods.pendingKaoya(pid, account).call();
}

export const getEarnedBusd = async (masterChefContract, account) => {
  return masterChefContract.methods.pendingReward(account).call();
}
...
export const stakeBusd = async (masterChefContract, amount, account) => {
  const decimal = 18;
  return masterChefContract.methods
    .deposit(
      new BigNumber(amount).times(new BigNumber(10).pow(decimal)).toString()
...
export const unstakeBusd = async (masterChefContract, amount, account, stake) => {
...
export const harvestBusd = async (busdStakingContract, account) => {
  return busdStakingContract.methods
    .deposit('0')
    .send({ from: account })
    .on('transactionHash', (tx) => {
      console.log(tx)
...
export const getStakedBusd = async (masterChefContract, account) => {
  try {
    const { amount } = await masterChefContract.methods
      .userInfo(account)
      .call()
...
export const getUserInfo = async (masterChefContract, account) => {
  try {
    const info = await masterChefContract.methods
      .userInfo(account)
      .call()
    return info
  } catch {
...
export const checkAirdrop = async (masterChefContract, depositTime) => {
  try {
    const info = await masterChefContract.methods
      .checkAirdrop(depositTime)
      .call()
    return info
  } catch {
    return false
...
export const getAirdrop = async (masterChefContract, account) => {
  return masterChefContract.methods
    .getAirdrop()
    .send({ from: account })
    .on('transactionHash', (tx) => {
