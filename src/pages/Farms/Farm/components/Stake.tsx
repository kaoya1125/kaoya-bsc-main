...
const Stake: React.FC<StakeProps> = ({ lpContract, pid, tokenName }) => {
  ...
  const handleApprove = useCallback(async () => {
    try {
      ...
      const estimatedGas = await tokenContract.estimateGas.approve(masterChefContract.options.address, ethers.constants.MaxUint256).catch(() => {
        // general fallback for tokens who restrict approval amounts
        return tokenContract.estimateGas.approve(masterChefContract.options.address, ethers.constants.MaxUint256)
      })
      ...
      return tokenContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            ...
            approval: { tokenAddress: tokenContract.address, spender: masterChefContract.options.address }
          })
        })
        .catch((error: Error) => {
