...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.HECO]).toBeTruthy()
      expect(txs[ChainId.HECO]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.HECO]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.THECO]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.HECO), String(ChainId.THECO)])
      expect(Object.keys(store.getState()[ChainId.HECO] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.THECO] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.HECO }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.HECO), String(ChainId.THECO)])
      expect(Object.keys(store.getState()[ChainId.HECO] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.THECO] ?? {})).toEqual(['0x1'])
    })
  })
})
